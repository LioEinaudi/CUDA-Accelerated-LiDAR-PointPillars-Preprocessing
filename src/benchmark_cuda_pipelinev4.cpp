...
{
    if (argc < 2) {
        std::cerr << "Usage: ./benchmark_cuda_pipelinev4 path/to/000000.bin\n";
        return 1;
    }
    ...
    try {
        ...
        std::cout << "CUDA modular V4:" << std::endl;
        ...
        std::cout << "feature:        " << modular_feature_ms << " ms" << std::endl;
        std::cout << "bev:            " << modular_bev_ms << " ms" << std::endl;
        ...
        std::cout << "feature values: " << modular_features.features.size() << std::endl;
        std::cout << "bev values:     " << modular_bev.data.size() << std::endl;
        std::cout << std::endl;

        std::cout << "CUDA pipeline V4:" << std::endl;
        ...
        std::cout << "feature copy:   skipped" << std::endl;
        std::cout << "BEV copy:       skipped" << std::endl;
        std::cout << std::endl;

        if (pipeline_total_ms > 0.0) {
            std::cout << "Speedup over modular V4: "
        ...
        if (message.find("no CUDA-capable device") != std::string::npos) {
            std::cout << "benchmark_cuda_pipelinev4 skipped: no CUDA-capable device" << std::endl;
            return 0;
        }
        throw;
    }

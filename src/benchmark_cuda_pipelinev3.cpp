...
{
    if (argc < 2) {
        std::cerr << "Usage: ./benchmark_cuda_pipelinev3 path/to/000000.bin\n";
        return 1;
    }
    ...
    try {
        ...
        (void)warmup_features;
        auto warmup_summary = cudaPreprocessPipelineV3(points, range, pillar);
        (void)warmup_summary;
        ...
        t0 = std::chrono::high_resolution_clock::now();
        auto pipeline_summary = cudaPreprocessPipelineV3(points, range, pillar);
        t1 = std::chrono::high_resolution_clock::now();
        double pipeline_total_ms = elapsedMs(t0, t1);

        std::cout << "CUDA modular V3:" << std::endl;
        ...
        std::cout << "CUDA pipeline V3:" << std::endl;
        ...
        if (pipeline_total_ms > 0.0) {
            std::cout << "Speedup over modular V3: "
        ...
        if (message.find("no CUDA-capable device") != std::string::npos) {
            std::cout << "benchmark_cuda_pipelinev3 skipped: no CUDA-capable device" << std::endl;
            return 0;
        }
        throw;
    }

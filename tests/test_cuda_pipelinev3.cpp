...
int main()
{
    ...
    std::vector<PointXYZI> points = {
        ...
        {2.05f, -39.55f, 0.0f, 0.5f},
        {-1.0f, -39.60f, 0.0f, 0.6f},
        {1.00f, -39.60f, 2.0f, 0.7f}
    };
    ...
    try {
        summary = cudaPreprocessPipelineV3(points, range, pillar);
    } catch (const std::runtime_error &error) {
        std::string message = error.what();
        if (message.find("no CUDA-capable device") != std::string::npos) {
            std::cout << "test_cuda_pipelinev3 skipped: no CUDA-capable device" << std::endl;
            return 0;
        }
        throw;
    }
    ...
    std::cout << "test_cuda_pipelinev3 passed" << std::endl;
    return 0;
}
